...
function getSessionId(): string {
  ...
  const key = `navio_embed_session_${window.location.pathname}`;
  let sessionId = sessionStorage.getItem(key);

  if (!sessionId) {
    sessionId = `embed_session_${Date.now()}_${Math.random().toString(36).substring(2, 15)}`;
...
// Track analytics event with embed flag
  ...
  try {
    await fetch("/api/public/analytics/event", {
      ...
      body: JSON.stringify({
        ...
        eventType,
        sessionId,
        embed: isEmbed, // Track that this is an embed view
      }),
    });
  } catch (error) {
...
export default function EmbedFlowViewer({
  flow,
  shareId,
}: EmbedFlowViewerProps) {
  ...
  useEffect(() => {
    if (!hasTrackedViewRef.current && sessionId) {
      ...
      trackEvent(flow.id, shareId, "VIEW", sessionId, true);
    }
  ...
  useEffect(() => {
    if (
      ...
      trackEvent(flow.id, shareId, "FLOW_COMPLETE", sessionId, true);
    }
  ...
  // Embed-optimized layout: no header, minimal padding, optimized for iframe
  return (
    <div className="bg-background flex min-h-[400px] flex-col">
      {/* Main Content - No header for cleaner embed experience */}
